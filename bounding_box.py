"""
Create bounding box coordinates of size 144 x 144 x 144.
"""
import numpy as np
...
patient_info_path = '/data/pg-umcg_mii/data/hecktor2022/hecktor2022_patient_info_training.csv'
pet_path = '/data/pg-umcg_mii/data/hecktor2022/imagesTr/'
bb_save_path = '/data/pg-umcg_mii/data/hecktor2022/src/bb_box/bb_box_training.csv'

bb_df = pd.DataFrame(columns=['PatientID', 'x1', 'x2', 'y1', 'y2', 'z1', 'z2'])

patients_list = list(pd.read_csv(patient_info_path)['PatientID'])
for pID in patients_list:
    bb = bbox_auto(sitk.ReadImage(pet_path + str(pID) + '__PT.nii.gz'))
    print(bb)
    bb_df = bb_df.append(
        {
            'PatientID': pID,
